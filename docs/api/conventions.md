...
- A folder listing also has `total`, the number of items over all pages. To jump to any part of a large folder, as a scrolling list does, the client can pass `offset=<position>` instead of `cursor` (not both: `400`). Items added or removed meanwhile can shift positions, while a cursor never skips or repeats an item.
...
Several items, or a folder, download as one ZIP file in two steps (S02.4-T03):

- `POST /api/v1/files/archives` with `{"paths": [...], "name": "…"}` checks every item and everything inside the folders before any byte is sent. Each item must exist; a link anywhere inside is refused (`400`), because links are never followed. One archive holds at most 100,000 files and folders (`422 too_large_for_sync`), and one request names at most 10,000 paths. The answer is `201` with a ticket: `id`, `url`, `name`, `expires_at`, `entries`, and `size`.
- `GET /api/v1/files/archives/{id}` streams the archive as an attachment, so a browser downloads it like any file. A ticket lasts 5 minutes and can be downloaded more than once. Tickets are kept in memory, so a restart forgets them (`404`).
- Entries are **stored**, not compressed: that is fast, and most large files are compressed already. Large files get ZIP64, names are UTF-8, and each entry keeps its modification time. Each item is at the top of the archive under its own name; a repeated name gets a number, as `name (1).ext`.
- Windows Explorer and Info-ZIP `unzip` open these archives, names included. Windows' own `tar.exe` skips names that its code page cannot hold (such as Arabic on an English system); that limit is the tool's, and it has the same limit with any ZIP file.
- Files are opened as a download opens them, so a link swapped in is never read, and a file removed since the ticket was made is left out. The server's memory stays bounded whatever the sizes; at log level `debug`, each finished archive logs `archive sent` with the heap size before and after (`heap_start_bytes`, `heap_end_bytes`). An error after the stream has started ends the connection, so the download shows as failed instead of looking complete.
...
- Each request body may be at most `uploads.max_chunk_size`; the whole upload at most `uploads.max_file_size`. Every answer of the upload endpoint, including the tus `OPTIONS` answer, names the chunk limit in the header `Upload-Max-Chunk-Size`, so a client can size its requests (S02.4-T01). A request that declares a larger body (`Content-Length`) or a larger upload (`Upload-Length`) gets `413 too_large` before any byte is stored; a body without `Content-Length` is cut at the limit.
...
| `GET /api/v1/files/items` | 1–7 met. Path through the resolver (3); paging values checked before the service, which a fake-service test proves (5); `offset` and `total` added in S02.3-T02 (a compatible addition): `offset` is checked in the handler and in the service (negative, or with `cursor`: `400`), and its tests come in S02.8 (RULES R6); no request body (6 n/a); errors 400/404/405/500 in the spec and in the contract test (2, 4); folder, file, paging, and error tests plus FuzzAPI (7). |
...
| `POST /api/v1/files/archives` | 1–7 met with R6 (S006): path checks through the files service (resolver, links refused, temporary names hidden) (3); strict JSON body, with the number of paths checked before the service (5); the body limit is 8 MiB for 10,000 paths (6); errors 400/404/405/413/422/500 in the spec, and the guard test's error case added (2, 4); its own tests come in S02.8 (7). Added in S02.4-T03. |
| `GET /api/v1/files/archives/{id}` | 1–7 met with R6 (S006): no path input, only a ticket ID; the files are opened as a download opens them (3, 5); the ZIP is streamed with one 1 MiB buffer, never read whole (6); errors 404/405/500 in the spec, and the guard test's error case added (2, 4); a failure after the start ends the connection (4); its own tests come in S02.8 (7). Added in S02.4-T03. |
| `DELETE /api/v1/files/items` | 1–7 met. Path through the resolver; temporary names are not found; links are deleted, never followed, and a path through a link to the outside is `outside_root` (3); `path` and the boolean `recursive` are bound and checked before the service, which a fake-service test proves (5); no request body (6 n/a); errors 400/404/405/409/500 in the spec and in the contract test (2, 4); files, empty and full folders, the root, unfinished uploads, links (Linux), hooks, and FuzzAPI (7). |
| `/api/v1/files/uploads/…` (tus, S01.4) | 1–7 met. Under `/api/v1/files` (1); the external tus protocol is documented in the spec under the `uploads` tag with every status it answers, which `TestTusStatusesAreInTheSpec` checks (2); the target goes through the files service's checks when the upload is created and again when it is finished (resolver, name rules, links, parent, conflict policy, free space) (3); every error is a problem, tusd's own protocol errors converted with their status kept (4); the metadata is checked before anything is stored (5); bodies stream to the store, the chunk and file limits apply before storing, and the memory bound is tested at 10 GiB (6); resume at any point, finalize with faults, checksums, cleanup, and FuzzAPI with a real tus server (7). |
