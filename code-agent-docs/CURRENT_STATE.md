...
## Next steps
1. **S01.1-T05** (CI): `.github/workflows/ci.yml` (lint on ubuntu; tests on ubuntu with `-race` and on windows; `scripts/coverage.sh`; govulncheck; `scripts/check-licenses.sh`; spec drift; cross-builds linux/amd64, linux/arm64, windows/amd64; dev image build + Trivy, which needs T06's Dockerfile) and `.github/dependabot.yml`. Branch `feat/S01.1-T05-ci`. Verify on GitHub: CI green on the PR into `develop`, and a deliberate failure turns it red. This also covers the CI halves of T03 and T04.
