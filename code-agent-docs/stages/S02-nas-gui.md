...
| S02.6 | File previews | **Done** (S006) | S02.3, S01.3 | FR-082, NFR-022 |
| S02.7 | Responsiveness and accessibility | In Progress | S02.2–S02.6 | NFR-015 |
...
| Task ID | Description | Status | Acceptance criteria |
|---|---|---|---|
| S02.7-T01 | **Phone and tablet layouts:** from 360 px up; the bottom navigation; 44 px touch targets; long-press menus; full-screen dialogs on phones. | **Done** (S006 E040) | Every flow works in the browser's device emulation at 360, 768, and 1024 px. |
...
| 2026-09-25 | S006 | **S02.6-T04 decision: pdf.js** (`pdfjs-dist` 6.3.289, Apache-2.0) **over the browser's built-in viewer.** <br>• **Why:** the download address answers with `Content-Disposition: attachment` and a sandbox CSP, so a built-in viewer in a frame downloads the file instead of showing it. It would need a second, weaker "inline" endpoint, and phones often have no viewer. pdf.js draws on a canvas in the page, and its core never runs a PDF's JavaScript: that needs the viewer's scripting sandbox, which the app does not load. pdf.js 6 has no `eval` at all (the old `isEvalSupported` option is gone). <br>• **How:** loaded only when a PDF opens (a separate chunk, and the worker as an app asset); WebAssembly off, with the JavaScript image decoders for JBIG2 and JPEG 2000 instead; CMaps and fonts from the app; one page at a time; reading by ranges without prefetching (a 62.9 MB PDF: 5.9 MB read for pages 1 and 60). <br>• **CSP needs, recorded for S03.5:** `worker-src 'self'` (a module worker from the app), `connect-src 'self'` (the PDF, CMaps, fonts), and `script-src 'self'` (the decoders' dynamic `import`); no `wasm-unsafe-eval`. What is lost without WebAssembly: ICC colour profiles are not applied (approximate colours in some PDFs). <br>• **Licenses:** only the CMaps, the Foxit fonts, and the two decoders are copied (all BSD or Apache). The Liberation Sans fonts in the package are GPL-2.0 with a font exception and are not shipped (`docs/licensing.md`). <br>• **Bug S02-B10** (found in S02.6-T04, fixed): in the preview, a button that turned disabled while it had the focus (Previous page on page 1, the last file's Next) dropped the focus to the page. The arrow keys then stopped stepping, because the frame read them on the dialog only. Fix: the preview reads its keys on the window while it is open, and the Files page's shortcuts stand down while any dialog is open. **Regression test for S02.8:** page to the start with the button, then ArrowRight steps to the next file | S02.6-T04 decision (4.2) | None |
| 2026-09-25 | S006 | **S02.7-T01 design details** (phones and tablets): <br>• **Touch:** as in phones' file managers, a tap opens an item (a folder, or a file's preview); a long press selects it and opens its menu; while something is selected, taps add or remove items. The mouse and keyboard keep click-to-select (S02.5-T01). <br>• **Touch targets:** every control is at least 44 px on a touch screen (`pointer-coarse`), and mouse layouts are unchanged. <br>• **Phones (under 640 px):** the toolbar keeps Upload and the view switch and puts Upload folder, New folder, and Download folder in a "More" menu. The selection bar keeps the count, Download, and Delete as icons, and puts Rename, Move, Copy, and Select all in a "More" menu. <br>• **Full-screen dialogs on phones** are the ones holding a list or several choices (the folder picker, the conflict dialog, the shortcuts). Short confirmations and name fields stay cards, as phone platforms do. <br>• The preview gives its content the whole width on phones, with the arrows over the edges | S02.7-T01 | None |
