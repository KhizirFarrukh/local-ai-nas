...
| S01.4 | Large file handling | In Progress | S01.2, S01.3, S01.5 | FR-004, FR-074, NFR-006, NFR-021 |
...
| S01.4-T05 | Size limits: `max_file_size` and `max_chunk_size` in config; tusd `MaxSize` set; simple uploads also enforce them. | Not started (the simple-upload part is done in S01.3-T05) | Over-limit uploads get 413 with no data stored (tus and simple upload). |
